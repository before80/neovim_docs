...
Prepend a value to string-style options. See [:set^=](https://neovim.io/doc/user/options.html#%3Aset^%3D)
...
Use to do a one-shot timer that calls `{fn}` **Note:** The `{fn}` is [vim.schedule_wrap()](https://neovim.io/doc/user/lua.html#vim.schedule_wrap())ped automatically, so API functions are safe to call.
...
vim.notify_once(`{msg}`, `{level}`, `{opts}`) `vim.notify_once()`Displays a notification only one time.
...
- `{fn}` will not be cleared by [nvim_buf_clear_namespace()](https://neovim.io/doc/user/api.html#nvim_buf_clear_namespace())

Parameters:

`{fn}` (`fun(key: string, typed: string)?`) Function invoked on every key press. [i_CTRL-V](https://neovim.io/doc/user/insert.html#i_CTRL-V) `{key}` is the key after mappings have been applied, and `{typed}` is the key(s) before mappings are applied, which may be empty if `{key}` is produced by non-typed keys. When `{fn}` is nil and `{ns_id}` is specified, the callback associated with namespace `{ns_id}` is removed.

`{ns_id}` (`integer?`) Namespace ID. If nil or 0, generates and returns a new [nvim_create_namespace()](https://neovim.io/doc/user/api.html#nvim_create_namespace()) id.
...
Paste handler, invoked by [nvim_paste()](https://neovim.io/doc/user/api.html#nvim_paste()) when a conforming UI (such as the [TUI](https://neovim.io/doc/user/term.html#TUI)) pastes text into the editor.
...
`vim.show_pos()`Show all the items at a given buffer position.
...
- [lua-patterns](https://neovim.io/doc/user/luaref.html#lua-patterns)
...
Empty table `{}` is a list, unless it was created by [vim.empty_dict()](https://neovim.io/doc/user/lua.html#vim.empty_dict()) or returned as a dict-like [API](https://neovim.io/doc/user/api.html#API) or Vimscript result, for example from [rpcrequest()](https://neovim.io/doc/user/builtin.html#rpcrequest()) or [vim.fn](https://neovim.io/doc/user/lua.html#vim.fn).
...
[vim.tbl_contains()](https://neovim.io/doc/user/lua.html#vim.tbl_contains()) for checking values in general tables
...
Escapes magic chars in [lua-patterns](https://neovim.io/doc/user/luaref.html#lua-patterns).
...
[vim.list_contains()](https://neovim.io/doc/user/lua.html#vim.list_contains()) for checking values in list-like tables
...
Form 1 validates that argument `{name}` with value `{value}` has the type `{type}`. `{type}` must be a value returned by [lua-type()](https://neovim.io/doc/user/luaref.html#lua-type()). If `{optional}` is true, then `{value}` may be null. This form is significantly faster and should be preferred for simple cases.
...
- (`table[]`) A list of objects with the following fields:
  ...
  - `{stat}` (`uv.uv_fs_t`) The fs_stat of the module path. Won't be returned for `modname="*"`
...
- `{opts}` (`table?`) Additional options. See [input()](https://neovim.io/doc/user/builtin.html#input())
...
**Note:** this uses [nvim_get_option_value()](https://neovim.io/doc/user/api.html#nvim_get_option_value()) but caches the result. This means [ftplugin](https://neovim.io/doc/user/usr_41.html#ftplugin) and [FileType](https://neovim.io/doc/user/autocmd.html#FileType) autocommands are only triggered once and may not reflect later changes.
...
- (`function?`) A function that modifies buffer state when called (for example, to set some filetype specific buffer variables). The function accepts a buffer number as its only argument.
...
Adds a new [mapping](https://neovim.io/doc/user/map.html#mapping). Examples:
...
- `{mode}` (`string|string[]`) Mode short-name, see [nvim_set_keymap()](https://neovim.io/doc/user/api.html#nvim_set_keymap()). Can also be list of modes to create mapping on multiple modes.

- `{lhs}` (`string`) Left-hand side [{lhs}](https://neovim.io/doc/user/map.html#{lhs}) of the mapping.

- `{rhs}` (`string|function`) Right-hand side [{rhs}](https://neovim.io/doc/user/map.html#{rhs}) of the mapping, can be a Lua function.

- `{opts}` (`table?`) Table of [:map-arguments](https://neovim.io/doc/user/map.html#%3Amap-arguments). Same as [nvim_set_keymap()](https://neovim.io/doc/user/api.html#nvim_set_keymap()) `{opts}`, except:
...
- [maparg()](https://neovim.io/doc/user/builtin.html#maparg())

- [mapcheck()](https://neovim.io/doc/user/builtin.html#mapcheck())

- [mapset()](https://neovim.io/doc/user/builtin.html#mapset())
...
- `{path}` (`string`) An absolute or relative path to the directory to iterate over. The path is first normalized [vim.fs.normalize()](https://neovim.io/doc/user/lua.html#vim.fs.normalize()).
...
(`string[]`) Normalized paths [vim.fs.normalize()](https://neovim.io/doc/user/lua.html#vim.fs.normalize()) of all matching items
...
If the buffer is unnamed (has no backing file) or has a non-empty ['buftype'](https://neovim.io/doc/user/options.html#'buftype') then the search begins from Nvim's [current-directory](https://neovim.io/doc/user/editing.html#current-directory).
...
- `{source}` (`integer|string`) Buffer number (0 for current buffer) or file path (absolute or relative to the [current-directory](https://neovim.io/doc/user/editing.html#current-directory)) to begin the search from.
...
Parses a raw glob into an [lua-lpeg](https://neovim.io/doc/user/lua.html#lua-lpeg) pattern.
...
(`vim.lpeg.Pattern`) pattern An [lua-lpeg](https://neovim.io/doc/user/lua.html#lua-lpeg) representation of the pattern
...
The LPeg library for parsing expression grammars is included as `vim.lpeg` (https://www.inf.puc-rio.br/~roberto/lpeg/).

In addition, its regex-like interface is available as [vim.re](https://neovim.io/doc/user/lua.html#vim.re) (https://www.inf.puc-rio.br/~roberto/lpeg/re.html).
...
The `vim.re` module provides a conventional regex-like syntax for pattern usage within LPeg [vim.lpeg](https://neovim.io/doc/user/lua.html#vim.lpeg). (Unrelated to [vim.regex](https://neovim.io/doc/user/lua.html#vim.regex) which provides Vim [regexp](https://neovim.io/doc/user/pattern.html#regexp) from Lua.)
...
Match line `{line_idx}` (zero-based) in buffer `{bufnr}`. If `{start}` and `{end}` are supplied, match only this byte index range. Otherwise see [regex:match_str()](https://neovim.io/doc/user/lua.html#regex%3Amatch_str()). If `{start}` is used, then the returned byte indices will be relative `{start}`.
...
[:trust](https://neovim.io/doc/user/editing.html#%3Atrust)
...
The trust database is located at [$XDG_STATE_HOME](https://neovim.io/doc/user/starting.html#%24XDG_STATE_HOME)/nvim/trust.
...
The `vim.version` module provides functions for comparing versions and ranges conforming to the [https://semver.org](https://semver.org/) spec. Plugins, and plugin managers, can use this to check available tools and dependencies on the current system.
...
A version "range spec" defines a semantic version range which can be tested against a version, using [vim.version.range()](https://neovim.io/doc/user/lua.html#vim.version.range()).
...
Parses and compares two version objects (the result of [vim.version.parse()](https://neovim.io/doc/user/lua.html#vim.version.parse()), or specified literally as a `{major, minor, patch}` tuple, e.g. `{1, 0, 3}`).
...
Returns `true` if the given versions are equal. See [vim.version.cmp()](https://neovim.io/doc/user/lua.html#vim.version.cmp()) for usage.
...
Returns `true` if `v1 >= v2`. See [vim.version.cmp()](https://neovim.io/doc/user/lua.html#vim.version.cmp()) for usage.
...
vim.version.le(`{v1}`, `{v2}`) `vim.version.le()`Returns `true` if `v1 <= v2`. See [vim.version.cmp()](https://neovim.io/doc/user/lua.html#vim.version.cmp()) for usage.
...
Parses a semver [version-range](https://neovim.io/doc/user/lua.html#version-range) "spec" and returns a range object:
...
`vim.iter()` is an interface for [iterable](https://neovim.io/doc/user/lua.html#iterable)s: it wraps a table or function argument into an `Iter` object with methods (such as [Iter:filter()](https://neovim.io/doc/user/lua.html#Iter%3Afilter()) and [Iter:map()](https://neovim.io/doc/user/lua.html#Iter%3Amap())) that transform the underlying source data. These methods can be chained to create iterator "pipelines": the output of each pipeline stage is input to the next stage. The first stage depends on the type passed to `vim.iter()`:

- Lists or arrays ([lua-list](https://neovim.io/doc/user/lua.html#lua-list)) yield only the value of each element.
  ...
  - Use [Iter:enumerate()](https://neovim.io/doc/user/lua.html#Iter%3Aenumerate()) to also pass the index to the next stage.
    - Or initialize with ipairs(): `vim.iter(ipairs(…))`.
  - Non-list tables ([lua-dict](https://neovim.io/doc/user/lua.html#lua-dict)) yield both the key and value of each element.
  - Function [iterator](https://neovim.io/doc/user/lua.html#iterator)s yield all values returned by the underlying function.
  - Tables with a [__call()](https://neovim.io/doc/user/luaref.html#__call()) metamethod are treated as function iterators.

The iterator pipeline terminates when the underlying [iterable](https://neovim.io/doc/user/lua.html#iterable) is exhausted (for function iterators this means it returned nil).

**Note:** `vim.iter()` scans table input to decide if it is a list or a dict; to avoid this cost you can wrap the table with an iterator e.g. `vim.iter(ipairs({…}))`, but that precludes the use of [list-iterator](https://neovim.io/doc/user/lua.html#list-iterator) operations such as [Iter:rev()](https://neovim.io/doc/user/lua.html#Iter%3Arev())).
...
For functions with side effects. To modify the values in the iterator, use [Iter:map()](https://neovim.io/doc/user/lua.html#Iter%3Amap()).
...
Flattens a [list-iterator](https://neovim.io/doc/user/lua.html#list-iterator), un-nesting nested values up to the given `{depth}`. Errors if it attempts to flatten a dict-like value.
...
`{depth}` (`number?`) Depth to which [list-iterator](https://neovim.io/doc/user/lua.html#list-iterator) should be flattened (defaults to 1)
...
If `n` is negative, offsets from the end of a [list-iterator](https://neovim.io/doc/user/lua.html#list-iterator).
...
`{n}` (`number`) Index of the value to return. May be negative if the source is a [list-iterator](https://neovim.io/doc/user/lua.html#list-iterator).
...
Gets the next value in a [list-iterator](https://neovim.io/doc/user/lua.html#list-iterator) without consuming it.
...
"Pops" a value from a [list-iterator](https://neovim.io/doc/user/lua.html#list-iterator) (gets the last value and decrements the tail).
...
Reverses a [list-iterator](https://neovim.io/doc/user/lua.html#list-iterator) pipeline.
...
Gets the first value satisfying a predicate, from the end of a [list-iterator](https://neovim.io/doc/user/lua.html#list-iterator).
...
Gets the last value of a [list-iterator](https://neovim.io/doc/user/lua.html#list-iterator) without consuming it.
...
Discards `n` values from the end of a [list-iterator](https://neovim.io/doc/user/lua.html#list-iterator) pipeline.
...
Sets the start and end of a [list-iterator](https://neovim.io/doc/user/lua.html#list-iterator) pipeline.
...
The generated table is an array-like table with consecutive, numeric indices. To create a map-like table with arbitrary keys, use [Iter:fold()](https://neovim.io/doc/user/lua.html#Iter%3Afold()).
...
- `direction` (vim.snippet.Direction): Navigation direction. Will return `true` if the snippet can be jumped in the given direction. See [vim.snippet.ActiveFilter](https://neovim.io/doc/user/lua.html#vim.snippet.ActiveFilter).
...
Tabstops are highlighted with [hl-SnippetTabstop](https://neovim.io/doc/user/syntax.html#hl-SnippetTabstop).
...
Converts the buffer shown in the current window to HTML, opens the generated HTML in a new split window, and saves its contents to `{file}`. If `{file}` is not given, a temporary file (created by [tempname()](https://neovim.io/doc/user/builtin.html#tempname())) is used.
...
- `{opt}` (`table?`) Optional parameters.
  ...
  - `{width}` (`integer`, default: ['textwidth'](https://neovim.io/doc/user/options.html#'textwidth') if non-zero or window width otherwise) Width used for items which are either right aligned or repeat a character infinitely.
